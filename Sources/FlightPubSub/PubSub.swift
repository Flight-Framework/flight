/// Topic-based publish/subscribe (§2). This is the whole surface: consumers
/// (Channels, Presence, Live) code against this protocol and never know
/// whether they are on one node or twenty (§5).
///
/// Semantics (§8, the non-goals, stated positively):
...
/// - **Exact-match topics** (§3). No wildcards in v1.
public protocol PubSub: Sendable {
    ...
    /// native, no manual unsubscribe bookkeeping required (§2.1).
    ///
