...
@Suite("Module DAG resolution (§7 step 5)")
struct ModuleGraphTests {
    ...
    func dependenciesFirst() throws {
        let order = names(try resolveModuleOrder([FakeServerModule.self, LoggingModule.self]))
    ...
    func diamond() throws {
        let order = names(try resolveModuleOrder([ModD.self]))
    ...
    func determinism() throws {
        let first = names(try resolveModuleOrder([ModD.self, LoggingModule.self]))
        for _ in 0..<10 {
            let again = names(try resolveModuleOrder([ModD.self, LoggingModule.self]))
            #expect(again == first)
        }
    ...
    func duplicates() throws {
        let order = names(try resolveModuleOrder([ModA.self, ModA.self, ModB.self]))
